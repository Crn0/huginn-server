...
import { followUsersByUsername } from "@/v1/user/service/follow-service.js";
...
beforeAll(async () => {
  const [login, users] = await Promise.all([
    ...
    Promise.all(
      Array.from({ length: 40 }).map(async (_, index) =>
        createUser({
          email: `user${index}.get@following.com`,
          displayName: `user.following${index}`,
  ...
  await followUsersByUsername(authUser!.id, followUsernames);
