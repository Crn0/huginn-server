...
beforeAll(async () => {
  const form = {
    email: "userId.post@following.com",
    displayName: "userId.following",
  ...
  const [login, users] = await Promise.all([
    ...
    Promise.all(
      Array.from({ length: 40 }).map(async (_, index) =>
        createUser({
          email: `userId${index}.get@following.com`,
          displayName: `userId.following${index}`,
          password: "Crnocrno123",
          birthday: new Date(),
        })
      )
  ...
  await followUsersById(createdUser.id, followIds);
...
describe("GET /api/v1/users/:userId/following", () => {
  ...
  describe("Success cases", () => {
    it("returns a list of following, total and the next and previous href", async () => {
      const url = `${baseUrl}/${userId}/following` as const;
  ...
  describe("Failure cases", () => {
    describe("Validation  errors", () => {
      it("returns a Validation error when the userId is invalid", async () => {
        const url = `${baseUrl}/invalid-user-id/following`;
