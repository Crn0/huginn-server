...
beforeAll(async () => {
  const form = {
    email: "usergetusers.post@usergetusers.com",
    displayName: "userget.users.data",
  ...
  await Promise.all(
    Array.from({ length: 40 }).map(async (_v, index) =>
      createUser({
        email: `usergetusers${(index += 1)}.post@usergetusers.com`,
        displayName: `${(index += 1)}user.get.users.data`,
        password: "Crnocrno123",
        birthday: new Date(),
      })
    )
