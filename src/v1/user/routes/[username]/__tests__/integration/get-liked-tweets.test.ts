...
import { likeTweets } from "@/v1/tweet/repository/like.js";

let accessToken: string;
let unauthAccessToken: string;
let username: string;
...
beforeAll(async () => {
  const form = {
    email: "username.post@likedtweets.com",
  ...
  const [login, unauthUserLogin] = await Promise.all([
  ...
  unauthAccessToken = unauthUserLogin.body.token;
  ...
  const tweets = await Promise.all(
    Array.from({ length: 40 }).map(async (_v, index) =>
      createTweet({
        content: `test_like_tweet_content_${(index += 1)}`,
        authorId: user.id,
        media: [],
      })
    )
  ...
  createTweet({ content: "test_like_tweet_content_41", authorId: user.id, media: [] });
...
describe("GET /api/v1/users/:username/tweets", () => {
  ...
  describe("Success cases", () => {
    it("returns a list of tweets, total and the next and previous href", async () => {
      const url = `${baseUrl}/${username}/tweets?scope=likes` as const;
  ...
  describe("Failure cases", () => {
    describe("Forbidden errors", () => {
      it("returns a forbidden error when fetching another user's liked tweets", async () => {
        const url = `${baseUrl}/${username}/tweets?scope=likes` as const;

        const res = await userRequest.get(url).set("Authorization", `Bearer ${unauthAccessToken}`);

        expect(res.status).toBe(403);
        expect(res.body).toMatchObject({
          code: "FORBIDDEN_ERROR",
          message: "You can only view your own tweet likes.",
        });
      });
    });
